...
3. Work the checklist using the `syncing-upstream` skill: re-port changed files (fan out one
   agent per file with the upstream diff + current Rust port), write fresh ports for new files,
   remove deleted ones; adversarially review each.
4. Verify with `cargo nextest run` + the differential vectors for the touched files.

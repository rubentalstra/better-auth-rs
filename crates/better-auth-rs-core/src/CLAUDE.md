...
Local detail for the framework-agnostic core (Rust reimplementation of `@better-auth/core`). See the
root `AGENTS.md` for build/test commands and the reimplementation contract, and
...
- Every ported `.rs` names its origin: `//! Upstream reference: <path>.ts`. The `type`→`types`
...
`#[cfg(test)] #[path = "<stem>.test.rs"] mod <stem>_tests;`. Write **our own Rust behavior tests**
asserting secure, correct behavior — there is no TS-vs-Rust differential harness and we do not match
upstream byte formats. Where better-auth's `*.test.ts` has cases worth keeping (edge cases; parsing
tables for `host`/`ip`/`url`/`string`; etc.), translate them into Rust tests, and add thorough
edge-case coverage for security-sensitive logic. The idiom to copy is
